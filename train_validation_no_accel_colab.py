...
def parse_args():
    ...
    # ── Multi-dataset embedding arguments ────────────────────────────────────
    parser.add_argument(
        "--embeddings_dir", type=str, default="./audio_embeddings/",
        help=(
            "Comma-separated paths to directories containing precomputed BEATs "
            "audio embeddings. Each directory may hold a merged "
            "audio_embeddings.safetensors, per-class chunks "
            "(<Class>_chunk_*.safetensors in chunks/), or legacy .pt files. "
            "Multiple Kaggle Datasets can be combined: "
            "--embeddings_dir /kaggle/input/emb-part1,/kaggle/input/emb-part2"
        ),
    )
    parser.add_argument(
        "--embeddings_preload_all", type=_str2bool, default=True,
        help=(
            "True (default): load all audio embedding tensors into RAM at init "
            "(zero I/O during training). "
            "False: lazy mmap — each tensor is read on demand via LRU-cached "
            "file handles (use when total size exceeds available RAM)."
        ),
    )
    parser.add_argument(
        "--embeddings_max_sf_handles", type=int, default=8,
        help=(
            "LRU cache size for open safetensors file handles "
            "(only relevant when --embeddings_preload_all false). "
            "Increase to 16-32 when you have many small per-class chunk files."
        ),
    )
