...
def parse_args():
    ...
    # FIX-GUIDANCE: raised default to 7.5
    ...
    # FIX-CFG: default changed to "zeros" (true p(x), matches training CFG dropout)
    parser.add_argument(
...
def _resolve_checkpoint_path(explicit_path, output_dir, stem, label):
    """
    FIX-CKPT: Prefer best_model (lowest validation loss) over learned_embeds
    (final checkpoint, potentially overfit).

    From the previous run: best_model step=3750 vloss=0.1745 vs
    learned_embeds step=7500 vloss=0.1867 — best_model is clearly better.
    ...
    # 1. Explicit path provided and exists → use it directly
    ...
    # 2. best_model checkpoints — PRIORITISED over learned_embeds
    best_patterns = [
    ...
    # 3. Final learned_embeds (fallback)
    for c in [os.path.join(output_dir, f"{stem}.safetensors"),
        ...
        if os.path.exists(c):
            logger.warning(
                ...
                "to use the step-3750 checkpoint explicitly."
            )
            return c

    # 4. Step checkpoints in weights/
    step_patterns = [
...
def _build_uncond_embedding(args, base_model, tokenizer, t_a, weight_dtype, device):
    """
    FIX-CFG: Build unconditional embedding for CFG → [1, 77, output_size].

    'zeros' mode = silent audio + empty text → true p(x) baseline.
    Matches the ~3% full-zero CFG dropout used during training.

    'text_only' is kept for experimentation but is out-of-distribution
    for the current asymmetric dropout schema.
    """
    audio_dim    = 768 * 3
...
def inference(args):
    ...
    # FIX-T_A: probe and warn if mismatch with training stride
    t_a = _probe_audio_frame_count(test_dataset)

    # Unconditional embedding (built once)
    uncond_embeddings = _build_uncond_embedding(
        args, base_model, tokenizer, t_a, weight_dtype, device
    )   # [1, 77, 1024]

    # Conditional prompt tokenisation (constant across all samples)
    ...
    for step, batch in enumerate(test_dataloader):
        ...
        # FIX-T_A: warn loudly if T_a changed (mixed-stride datasets)
        ...
        with torch.no_grad():
            ...
            # CFG: stack [uncond, cond] → [2, 77, 1024]
